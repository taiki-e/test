...
for f in $(ls_files '*.sh'); do
    if grep -Eq '(^|[\( /])(grep) -E' "${f}"; then
        grep_ere_files+=("${f}")
    fi
    if grep -Eq '(^|[\( /])(sed) -E' "${f}"; then
        sed_ere_files+=("${f}")
    fi
done
if [[ ${#grep_ere_files[@]} -gt 0 ]]; then
    res=$(grep -En '(^|[\( /])(grep) ([^-]|-[^EFP])' "${grep_ere_files[@]}" | (grep -E -v '^[^ ]+: *#' || true) | LC_ALL=C sort || true)
    if [[ -n "${res}" ]]; then
        error "please always use ERE (-E flag) instead of BRE for code consistency within a file"
        printf "=======================================\n"
        printf "%s\n" "${res}"
        printf "=======================================\n"
    fi
...
if [[ ${#sed_ere_files[@]} -gt 0 ]]; then
    res=$(grep -En '(^|[\( /])(sed) ([^-]|-[^E])' "${sed_ere_files[@]}" | (grep -E -v '^[^ ]+: *#' || true) | LC_ALL=C sort || true)
    if [[ -n "${res}" ]]; then
        error "please always use ERE (-E flag) instead of BRE for code consistency within a file"
        printf "=======================================\n"
        printf "%s\n" "${res}"
...
if [[ "${ostype}" == "solaris" ]] && [[ -n "${CI:-}" ]] && ! type -P shellcheck &>/dev/null; then
    ...
    if [[ -n "$(ls_files '*Dockerfile*')" ]]; then
        ...
        if ! shellcheck --shell bash -e SC2154,SC2250 $(ls_files '*Dockerfile*'); then
            should_fail=1
        fi
    fi
    ...
    if [[ -d .github/workflows ]]; then
        info "running \`shellcheck -e SC2086,SC2096,SC2129\` for scripts in .github/workflows/*.yml"
        if check_install jq python3; then
            venv_install_yq
            for workflow in .github/workflows/*.yml; do
                ...
                if [[ "${default_shell}" != "bash --noprofile --norc -CeEuxo pipefail {0}" ]]; then
                    error "${workflow}: default shell must be 'bash --noprofile --norc -CeEuxo pipefail {0}'"
                    continue
                fi
                for job in $(yq -c '.jobs | to_entries | .[]' "${workflow}"); do
                    dir="tmp/tidy/shell/${workflow}"
                    name=$(jq -r '.key' <<<"${job}")
                    job=$(jq -r '.value' <<<"${job}")
                    n=1
...
EOF
                            case "${ostype}" in
                                windows) sed -i 's/\r//g' "${path}" ;;
                            esac
                            if ! shellcheck -e SC2086,SC2096,SC2129 "${path}" | sed "s/$(sed_rhs_escape "${path}")/$(sed_rhs_escape "${display_path}")"/g; then
                                should_fail=1
                            fi
                        }
                        emit_and_shellcheck "${run}" "${shell}" "${dir}/${name}.step${n}.run" "${dir#tmp/tidy/shell/} ${name}.step${n}.run"
                        emit_and_shellcheck "${prepare}" 'sh' "${dir}/${name}.step${n}.prepare" "${dir#tmp/tidy/shell/} ${name}.step${n}.run"
                        ((n++))
                    done
                done
            done
...
EOF
        ...
        for dictionary in .github/.cspell/*.txt; do
            ...
            case "${ostype}" in
                ...
                netbsd) dup=$(sed '/^$/d; /^\/\//d' "${project_dictionary}" "${dictionary}" | LC_ALL=C sort -f | tr '[:upper:]' '[:lower:]' | uniq -d) ;;
